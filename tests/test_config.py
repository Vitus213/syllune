...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"asr": {"final_backend": "none"}}, ValueError, "asr.final_backend 的值无效"),
        (
            {"asr": {"partial_interval_millis": 31}},
            ValueError,
            "asr.partial_interval_millis 必须在",
        ),
        (
            {"asr": {"partial_interval_millis": 5001}},
            ValueError,
            "asr.partial_interval_millis 必须在",
        ),
        (
            ...
            ValueError,
            "实时识别仅支持 capture.sample_rate = 16000",
        ),
        ({"capture": {"channels": 2}}, ValueError, "实时识别仅支持 capture.channels = 1"),
